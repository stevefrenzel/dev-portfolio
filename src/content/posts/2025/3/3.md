...
tags: ["a11y", "accessibility", "html", "aria"]
image:
...
If it’s not possible to use the visible text as accessible name, or if there’s no visible text at all, it might be a good use case for `aria-label`.
...
We could’ve also done it using the `alt` attribute, but in this case we would’ve needed to hide the visible text using `aria-hidden=”true”`, otherwise it could’ve been announced as “link, Wikipedia Wikipedia (external link)” or similar by assistive technology.
...
Assume we have a user interface where we can delete stuff by clicking a button. But because of limited real estate, we can only use the text “Delete” to describe it’s purpose. To give some more context for users of assistive technology, we could provide a [visually hidden](https://webaim.org/techniques/css/invisiblecontent/) description:
...
Let’s stick with the delete button example to showcase how precedence works with these three attributes. This is an example for showcase purposes only, please DO NOT use it in production!
